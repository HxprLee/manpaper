...
def build_command(backend, path, socket_path=None):
    """Builds the command to set a wallpaper with a given backend."""
    if backend == 'swaybg':
        return f'swaybg -i "{path}"'
    if backend == 'swww':
        return f'swww img "{path}"'
    if backend == 'hyprpaper':
        return f'hyprpaper --output * --set "{path}"'
    if backend == 'mpvpaper':
        if not socket_path:
            return None
        return f'mpvpaper -vs -o "loop input-ipc-server={socket_path}" ALL "{path}"'
    return None
...
class PreferencesWindow:
    ...
    def _setup_slider_scroll_controller(self, slider):
        ...
        def on_scroll(controller, dx, dy):
            ...
            # dy is -1 for scroll up, 1 for scroll down. We want up to increase, so subtract dy.
    ...
    def create_preferences_view(self):
        ...
        backends_group = Adw.PreferencesGroup(title="Backends")
        preferences_page.add(backends_group)
        ...
        if not installed_static_backends:
            ...
            try:
                selected_index = installed_static_backends.index(current_static_backend)
                row_static.set_selected(selected_index)
            except ValueError:
        ...
        backends_group.add(row_static)

        video_group = None
        if is_backend_installed('mpvpaper'):
            video_group = Adw.PreferencesGroup(title="Video Settings", description="Requires mpvpaper and socat")
        ...
        if not installed_live_backends:
            ...
            def on_live_backend_changed(combo, _):
                ...
                if video_group:
                    video_group.set_visible(backend == 'mpvpaper')
            row_live.connect('notify::selected', on_live_backend_changed)
        backends_group.add(row_live)

        if video_group:
            preferences_page.add(video_group)
            video_group.set_visible(self.settings.get_string('live-backend') == 'mpvpaper')
        ...
        row_use_custom_css = Adw.SwitchRow(title="Use Custom CSS", active=self.app.use_custom_css)
        row_use_custom_css.connect('notify::active', self._on_use_custom_css_toggled)
        appearance_group.add(row_use_custom_css)

        row_select_custom_css = Adw.ActionRow(title="Custom CSS File Path")
        entry_custom_css_path = Gtk.Entry(hexpand=True, text=self.app.custom_css_path or "", margin_top=8, margin_bottom=8)
        entry_custom_css_path.connect("changed", self._on_custom_css_path_changed)
        row_select_custom_css.add_suffix(entry_custom_css_path)
        row_select_custom_css.set_activatable_widget(entry_custom_css_path)
        row_select_custom_css.set_sensitive(self.app.use_custom_css)
        appearance_group.add(row_select_custom_css)

        row_use_custom_css.bind_property("active", row_select_custom_css, "sensitive", Gio.SettingsBindFlags.DEFAULT)
        ...
        appearance_group.add(row_reload_css)
...
class Manpaper(Adw.Application):
    ...
    def _create_window(self):
        ...
        static_page = self.view_stack.add_titled(self._create_scrolled_window(self.static_view), 'static', 'Static Wallpapers')
        static_page.set_icon_name('image-x-generic-symbolic')

        self.live_view = self._create_grid_view(self.live_model)
        live_page = self.view_stack.add_titled(self._create_scrolled_window(self.live_view), 'live', 'Live Wallpapers')
        live_page.set_icon_name('video-x-generic-symbolic')
        
        prefs_view = self.prefs_window.create_preferences_view()
        prefs_page = self.view_stack.add_titled(prefs_view, 'preferences', 'Preferences')
    ...
    def _on_factory_setup(self, factory, list_item):
        ...
        click_controller.connect("pressed", self._on_list_item_right_clicked, list_item)
    ...
    def _on_factory_bind(self, factory, list_item):
        ...
        image_path = self._get_thumbnail_path(item)
        if image_path:
            if image_path not in self.texture_cache:
                try:
                    pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(image_path, self.preview_size, int(self.preview_size * self.aspect_ratio))
                    self.texture_cache[image_path] = Gdk.Texture.new_for_pixbuf(pixbuf)
                except GLib.Error as e:
                    print(f"Error loading image {image_path}: {e}")
                    self.texture_cache[image_path] = None
    ...
    def _get_thumbnail_path(self, item):
        """Gets the path for a thumbnail, generating it if needed."""
        if item.path.suffix.lower() in SUPPORTED_LIVE:
            thumb_path = self.cache_dir / (item.path.stem + '_thumb.jpg')
            if not thumb_path.exists() or thumb_path.stat().st_mtime < item.path.stat().st_mtime:
                try:
                    subprocess.run(['ffmpegthumbnailer', '-i', str(item.path), '-o', str(thumb_path), '-s', '256', '-q', '5'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                except Exception as e:
    ...
    def _on_wallpapers_loaded(self, static_paths, live_paths):
        ...
        self.static_store.remove_all()
        self.live_store.remove_all()
        self.texture_cache.clear()
        for file in static_paths: self.static_store.append(WallpaperItem(file))
        for file in live_paths: self.live_store.append(WallpaperItem(file))
        self.spinner.stop()
    ...
    def _set_wallpaper(self, item):
        ...
        cmd = build_command(backend, path, resolved_socket_path)
    ...
    def _perform_recode(self, item, window):
        ...
        output_path = str(item.path.with_name(f"{item.path.stem}_recoded{item.path.suffix}"))
    ...
    def _live_wallpaper_filter_func(self, item):
        ...
        if self.hide_original_after_recode and '_recoded' not in item.path.name:
            recoded_path = item.path.with_name(f"{item.path.stem}_recoded{item.path.suffix}")
